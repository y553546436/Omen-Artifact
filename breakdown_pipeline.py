...
def trainer_args(trainer, dtype):
    trainer_args_templates = {
        'LeHDC': '--dim 5056 -b -m -1 -M 1 -l 2',
        'LDC': f'{"--fd 256 --vd 4 -b" if dtype == "binary" else "--dim 256"}',
        'OnlineHD': '--dim 10048 -b -m -1 -M 1 -l 2', 
    }
    return f"'{trainer_args_templates[trainer]}'"


def make_command(dataset, dtype, trainer, start, freq, alpha, strategy, device):
    cmd = f'make -f Makefile-breakdown DATASET={dataset} DTYPE={dtype} TRAINER={trainer} TRAINER_ARGS={trainer_args(trainer, dtype)} START={start} FREQ={freq} ALPHA={alpha} STRATEGY={strategy} DEVICE={device}'
    return cmd
    ...
    for trainer in trainers:
        for dtype in dtypes:
            for dataset in datasets:
                if dtype == 'real' and trainer in ['LeHDC', 'OnlineHD']:
                    continue
                if dataset == 'language' and trainer != 'OnlineHD':
                    continue
                start = '512' if trainer in ['LeHDC', 'OnlineHD'] else '16'
                freq = '64' if trainer in ['LeHDC', 'OnlineHD'] else '4'
                for alpha in alphas:
                    for strategy in strategies:
                        commands.append(make_command(dataset, dtype, trainer, start, freq, alpha, strategy, args.device))
    return commands
