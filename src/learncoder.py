...
class Encoder(nn.Module):
    ...
    def __init__(self, features : int, dim : int = 4000):
        ...
        self.basis = nn.Parameter(torch.randn(self.dim, self.features))
        self.base = nn.Parameter(torch.empty(self.dim).uniform_(0.0, 2*math.pi))
    ...
    def forward(self, x : torch.Tensor):
        ...
        # temp = x @ self.basis.T
        # h = torch.sin(temp) * torch.cos(temp + self.base)
        # return h
