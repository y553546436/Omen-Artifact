...
def load():
    # fetches data
    x, y = sklearn.datasets.fetch_openml('mnist_784', return_X_y=True, parser='auto')
    x = np.array(x)
    y = np.array(y)
    x = x.astype(float)
    y = y.astype(int)
    ...
    # split and normalize
    x, x_test, y, y_test = sklearn.model_selection.train_test_split(x, y, random_state=0, stratify=y, test_size=1/7)
...
if __name__ == '__main__':
    ...
    if args.header:
        ...
        generate_test_data_header('mnist', args.header, x_test, y_test)
