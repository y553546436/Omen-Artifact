...
def make_command(dataset, dtype, trainer, start, freq, alpha, strategy):
    cmd = f'make -f local.mk DATASET={dataset} DTYPE={dtype} TRAINER={trainer} TRAINER_ARGS={trainer_args(trainer, dtype)} START={start} FREQ={freq} ALPHA={alpha} STRATEGY={strategy}'
    return cmd
