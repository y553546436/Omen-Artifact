...
int main(void)
{
  ...
  // Timer begins
  HAL_TIM_Base_Start(&htim16); // 250 kHz clock
//
//  for (int i = 0; i < NUMTEST; i++) {
//	  // Print UART start test
//	  uart_buf_len = sprintf(uart_buf, "Test #%u\r\n", i);
...
//	  // Show elapsed time
//	  timer_val /= 250; // convert clock cycles to ms
//	  uart_buf_len = sprintf(uart_buf, "Normal: %u ms, \r\n", timer_val);
//	  HAL_UART_Transmit(&huart1, (uint8_t *)uart_buf, uart_buf_len, 100);
//
//	  // Omen testing
//	  timer_val = __HAL_TIM_GET_COUNTER(&htim16);
...
//	  // Show elapsed time
//	  timer_val /= 250; // convert clock cycles to ms
//	  uart_buf_len = sprintf(uart_buf, "Omen: %u ms ", timer_val);
//	  HAL_UART_Transmit(&huart1, (uint8_t *)uart_buf, uart_buf_len, 100);
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
	{
	    if (huart->Instance == USART1)
	    {
          ...
          // HAL_UART_Transmit(&huart1, (uint8_t *)uart_buf, uart_buf_len, 100);
          HAL_UART_Transmit(&huart1, (uint8_t *)results, sizeof(results), 1000);

	        // Process the received data (e.g., print or use it in your application)
//	        HAL_UART_Transmit(&huart1, tx_message, sizeof(tx_message)-1, HAL_MAX_DELAY);

	        // Restart UART reception
	        HAL_UART_Receive_IT(&huart1, rx_buffer, BUFFER_SIZE);

	        //uart_buf_len = sprintf(uart_buf, "received buffer size %d\n", BUFFER_SIZE);
	        //HAL_UART_Transmit(&huart1, (uint8_t *)uart_buf, uart_buf_len, 100);
	    }
	}
/* USER CODE END 4 */
